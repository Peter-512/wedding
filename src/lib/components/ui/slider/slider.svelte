<script lang="ts">
	...
	let {
		...
		class: className,
		...restProps
	}: WithoutChildrenOrChild<SliderPrimitive.RootProps> & { size: number } = $props();
</script>
	...
	{#snippet children({ thumbs })}
		...
		{#each thumbs as thumb}
			<SliderPrimitive.Thumb
				...
				class="block rounded-full border-2 border-primary bg-background ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"
			/>
		{/each}
	{/snippet}
</SliderPrimitive.Root>
